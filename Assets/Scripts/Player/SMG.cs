		...
		if (shootable)
		{
			if(Input.GetMouseButton(0))
			{
				if (delay <= 0)
				{
					...
					if (Physics.Raycast(transform.position, transform.TransformDirection(new Vector3(reboundRay.x, reboundRay.y, 75)), out hit, 1000.0f, _layerMask)) 
					{
					...
					SMGRecoilFunction.instance.Recoil();
					AmmoFunction.instance.SMG_Capacity -= 1;
				}
			}
		}
