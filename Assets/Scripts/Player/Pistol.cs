...
public class Pistol : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		pistolShootAnim = GameObject.Find("Pistol").GetComponent<Animator>();
		...
		if (shootable)
		{
			if (Input.GetMouseButton(0))
			{
				if (delay <= 0)
				{
					...
					//if (Physics.Raycast(transform.position, transform.TransformDirection(new Vector3(reboundRay.x, reboundRay.y, 20)), out hit, 1000.0f, _layerMask)) 
					...
					PistolRecoilFunction.instance.Recoil();
					AmmoFunction.instance.Pistol_Capacity -= 1;
				}
			}
		}
	...
	{
		pistolShootAnim.SetTrigger("isShooting");
		...
		//gunModel.transform.Translate(Vector3.left * 0.1f);   //반동으로 0.3만큼 뒤로 밀린다.
